...
// tunes: Return average network hashes per second based on last number of blocks.
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "cryptune is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "cryptune is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "cryptune is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "cryptune is downloading blocks...");
