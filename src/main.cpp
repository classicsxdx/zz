...
unsigned int nStakeMaxAge = 60 * 60 * 24 * 16;	// stake age of full weight: 16d
unsigned int nStakeTargetSpacing = 512;			// 8min 31sec block spacing

int64 nChainStartTime = 1395946112;
...
const string strMessageMagic = "cryptune Signed Message:\n";
...
int64 GetProofOfWorkReward(int nHeight, int64 nFees, uint256 prevHash)
{
	...
	if(nHeight < 1476)	// 61 days 8x payout 
	{
		nSubsidy *= 8;
	}

	// Subsidy is cut in half every 302,404 blocks, which will occur approximately every XX? months
	nSubsidy >>= (nHeight / 302404); 
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in cryptune
    bool fStrictPayToScriptHash = true; // Always active in cryptune
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "cryptune", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "April 1st, 2014: No joke. Happy Easter/ I like turtles";
        CTransaction txNew;
		...
		txNew.strTxComment = "text:cryptune genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x191db19739ff73333f7559867619451c6e4844d9716b2377818b02356e883ac9"));
